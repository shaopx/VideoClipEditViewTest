...
class ClipContainer : FrameLayout {
        ...
        MotionEvent.ACTION_MOVE -> {
          ...
          if (xDistance != 0f) {
            ...
            if (width - v.width + newTransx < leftFrameLeft + leftFrameBar!!.width.toFloat() + minDistance) {
              newTransx =
                -(width.toFloat() - (leftFrameLeft + leftFrameBar!!.width.toFloat() + minDistance) - v.width.toFloat())
            }
  ...
  fun onPreviewChange(finished: Boolean) {
    ...
    var previewMillSec =
      (previewPosition - getFrameFixLeftX()) * 1f / frameWidth * millSecInFrame
...
//        Log.d(TAG, "adjustProgressBar  transX_:$transX_, transX: $transX")
  ...
  fun init(context: Context) {
    ...
    progressPaint = Paint()
    progressPaint!!.color =
      context.resources.getColor(R.color.video_clip_progress_color)
    ...
    shadowPaint!!.style = Paint.Style.FILL
    minDistance =
      context.resources.getDimensionPixelSize(R.dimen.video_clip_min_length)
        .toFloat()
    progressWidth =
      context.resources.getDimensionPixelSize(R.dimen.video_clip_progressbar_width)

    with(context.resources) {
      recyclerViewPadding =
        getDimensionPixelSize(R.dimen.clip_recyclerview_paddingleft)
      ...
      framebarPadding =
        getDimensionPixelSize(R.dimen.clip_frame_bar_width_outer) - getDimensionPixelSize(
          R.dimen.clip_frame_bar_width
        )
      framebarImageWidth = getDimensionPixelSize(R.dimen.clip_frame_bar_width)
      realProgressBarWidth =
        getDimensionPixelSize(R.dimen.clip_frame_progressbar_width)
      minProgressBarX =
        getDimensionPixelSize(R.dimen.clip_recyclerview_paddingleft)
    }
  }
...
//        clipFrameLayout = findViewById(R.id.clipContainer)
    ...
    recyclerview.layoutManager =
      androidx.recyclerview.widget.LinearLayoutManager(context).apply {
        orientation =
  ...
  fun updateInfo(mediaDutaion: Long, itemCount: Int) {
    ...
    if (mediaDutaion > maxSelection) {
      ...
      rightShadowStart =
        (rightFrameLeft + framebarImageWidth).toInt() - SHADOW_DELTA
...
//        rightFrameLeft = rightFrameBar.translationX
    maxProgressBarX =
      width - resources.getDimensionPixelSize(R.dimen.clip_recyclerview_paddingleft)
    frameWidth = width - leftFrameBar!!.width - rightFrameBar!!.width
  }
...
//            Log.d(TAG, "setProgress: millsecs:$millsecs, ratio:$ratio")
  ...
  private fun onFrameMoved(finished: Boolean) {
    ...
    startMillSec =
      (getCutLeftX() - getFrameFixLeftX()) * 1f / frameWidth * millSecInFrame
    endMillSec =
      (getCutRightX() - getFrameFixLeftX()) * 1f / frameWidth * millSecInFrame
...
//            Log.d(TAG, "onFrameMoved: leftShadowStart:$leftShadowStart, leftShadowEnd:$leftShadowEnd")
      ...
      Log.d(
        TAG,
        "onFrameMoved: rightShadowStart:$rightShadowStart, rightShadowEnd:$rightShadowEnd"
      )

      if (callback != null) {
        callback!!.onSelectionChang(
          itemCount,
          startMillSec.toLong(),
          endMillSec.toLong(),
          finished
    ...
    Log.d(
      TAG,
      "onFrameMoved: position:$position, itemLeft:$itemLeft,  scrollX:$scrollX"
    )
...
//        Log.e(TAG, "onFrameMoved: final startMillSec:$startMillSec, endMillSec:$endMillSec, range:${endMillSec - startMillSec}")
  ...
  override fun dispatchDraw(canvas: Canvas) {
    ...
    if (leftShadowEnd > leftShadowStart) {
      canvas.drawRect(
        Rect(leftShadowStart, 0, leftShadowEnd + 2, height),
        shadowPaint
      )
    }
    ...
    if (rightShadowEnd > rightShadowStart) {
      canvas.drawRect(
        Rect(rightShadowStart - 2, 0, rightShadowEnd, height),
        shadowPaint
      )
    }
    ...
    canvas.drawRect(
      Rect(
        (leftFrameLeft + leftFrameBar.width).toInt(),
        0, (rightFrameLeft + DELTA).toInt(), framebarHeight
      ), paint!!
    )
    canvas.drawRect(
      Rect(
        (leftFrameLeft + leftFrameBar.width).toInt(),
        height - framebarHeight, (rightFrameLeft + DELTA).toInt(), height
      ), paint!!
    )
  }
...
fun androidx.recyclerview.widget.RecyclerView.getScollXDistance(): Triple<Int, Int, Int> {
  var layoutManager =
    getLayoutManager() as androidx.recyclerview.widget.LinearLayoutManager
