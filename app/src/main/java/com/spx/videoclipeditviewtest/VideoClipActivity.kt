...
class VideoClipActivity : AppCompatActivity(), ClipContainer.Callback {
  ...
  lateinit var clipContainer: ClipContainer
  lateinit var pb_progress: ProgressBar
  lateinit var tv_framepreviewmode: TextView
  lateinit var tv_clip: TextView
  lateinit var toast_msg_tv: TextView
  ...
  lateinit var player_view_exo: StyledPlayerView
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    seekbar = findViewById(R.id.play_spped_seakbar)
    clipContainer = findViewById(R.id.clipContainer)
    pb_progress = findViewById(R.id.pb_progress)
    tv_framepreviewmode = findViewById(R.id.tv_framepreviewmode)
    tv_clip = findViewById(R.id.tv_clip)
    ...
    player_view_exo = findViewById(R.id.player_view_exo)

    initPlayer()

    seekbar.max = SPEED_RANGE
    var normalSpeed = seekbar.max / 2
    seekbar.progress = normalSpeed
    ...
    tv_clip.setOnClickListener {
      Log.d(
        TAG,
        "startMillSec:$startMillSec,  endMillSec:$endMillSec,  mediaDuration:$mediaDuration"
      )
...
//                        Log.d(TAG, "onProgress = $progress")
  ...
  private fun onProcessCompleted() {
    ...
    if (!file.exists()) {
      Toast.makeText(this, "请更新videoPlayUrl变量为本地手机的视频文件地址", Toast.LENGTH_LONG)
        .show()
    }
...
//        Log.d(TAG, "onPreviewChang   startMillSec:$startMillSec")
  ...
  override fun onSelectionChang(
    ...
    Log.d(
      TAG,
      "onSelectionChang ...startMillSec:$_startMillSec, endMillSec:$_endMillSec"
    )
    ...
    toast_msg_tv.text =
      "已截取${secFormat.format(selSec)}s, [$startMillSec - $endMillSec]"
    toast_msg_tv.visibility = View.VISIBLE
